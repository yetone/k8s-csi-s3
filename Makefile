...
IMAGE_NAME=bentoml/csi-s3
CSI_DRIVER_IMAGE_NAME=bentoml/csi-s3-driver
VERSION ?= 0.41.1
...
push: container
	...
	docker tag $(IMAGE_TAG) $(REGISTRY_NAME)/$(CSI_DRIVER_IMAGE_NAME):$(VERSION)
	docker tag $(IMAGE_TAG) $(REGISTRY_NAME)/$(CSI_DRIVER_IMAGE_NAME):latest
	docker push $(IMAGE_TAG)
	docker push $(REGISTRY_NAME)/$(IMAGE_NAME)
	docker push $(REGISTRY_NAME)/$(CSI_DRIVER_IMAGE_NAME)
	docker push $(REGISTRY_NAME)/$(CSI_DRIVER_IMAGE_NAME):$(VERSION)
clean:
	go clean -r -x
	-rm -rf _output
